var dataModule = 'app/storage/' + ( isPhoneGap() ? 'sqlite' : 'localstorage' );
define(['angular', 'app/levelsList', dataModule], function(angular) {
    'use strict';

    angular.module('app.data', ['app.levelsList', 'app.storage'])
            .factory('levelsData', $LevelsData)
            .factory('userLevelsData', $UserLevelsData);

    $LevelsData.$inject = ['util', 'levels', 'chapters'];
    function $LevelsData(u, levels, chapters) {
        return {
            ...
            getFullChapter: function(chapterId) {
                var chapter = u.clone(this.getChapter(chapterId));
    ...
    $UserLevelsData.$inject = ['playerData', 'levelsData', '$q'];
    function $UserLevelsData(playerData, levelsData, $q) {
        return {
